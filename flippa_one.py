...
done = df.drop_duplicates(subset=['Domain','Type','Category'],keep=False) # removing duplicate rows. index will remain same
dn = done.reset_index(drop=True) #droping previous index
dn.to_excel('flippa_all_listings_today.xlsx',encoding='utf-8', index=False) # saving as excel
